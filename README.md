...
.
|-- training.py
|-- training_with_side_camera.py  # Variant: adds left/right recovery samples
|-- TestSimulation.py
...
|-- model_side_camera.h5          # Generated by training_with_side_camera.py, not committed
|-- simulator-windows-64/         # Local simulator, not committed
|-- simulator-mac/                # Local simulator, not committed
|-- training_outputs/             # Generated plots and logs (training.py)
`-- training_outputs_side_camera/ # Generated plots and logs (training_with_side_camera.py)
```
...
`training_with_side_camera.py` is the exception: it also uses the left/right
columns as synthetic recovery samples. See
[Alternative Training Scripts](#alternative-training-scripts).
...
### `training_with_side_camera.py`

A controlled variant of `training.py` with exactly one change: left/right
camera images are added to the **training** split as synthetic recovery
samples (a left-camera shot is relabeled as if the car needs to steer more
right, and vice versa). Validation stays center-camera only, so validation
loss remains directly comparable to `training.py`'s. Everything else —
chunk-based splitting, steering-bin balancing, augmentation, model
architecture, callbacks — is identical.

```bash
python training_with_side_camera.py --epochs 20 --batch-size 800

# Sweep the recovery steering offset (default 0.2)
python training_with_side_camera.py --steering-correction 0.15
```

It writes to separate output paths so runs don't overwrite `training.py`'s:

```text
model_side_camera.h5
training_outputs_side_camera/steering_histogram.png
training_outputs_side_camera/training_history.png
training_outputs_side_camera/training_log.csv
```

Compare `model.h5` and `model_side_camera.h5` in `TestSimulation.py` to see
whether the side-camera recovery data actually improves driving behavior.
